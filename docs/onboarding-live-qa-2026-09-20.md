...
Status: partial live acceptance. The `gZRuKyGJ` candidate completed three native onboarding/save/reuse cycles with clean saved layouts, followed by three bounded Chrome scenarios in an isolated, unsigned-in profile. Chrome tab-ID policy, tab retention and same-tab navigation were observed in two scenarios; the all-tabs-blocked scenario does not establish physical input blocking. Only physical single-backtick picker entry and Esc closure are user-confirmed. The later `ksf8n4nv` Settings fix passed release/package/signature checks and app-directed live settings/reopen/guide-preservation checks. Its effective macOS permissions are not yet ready: replacing only the QA permission entry is paused at a Touch ID/password sheet. Browser fixtures remain installed pending cleanup, with no active QA intention. These observations do not constitute three fully integrated acceptance passes.
...
## Settings-candidate permission handoff and latest integrity checkpoint

The new `ksf8n4nv` binary showed the real permission gate, so effective grants are not claimed for it. Earlier grants to another ad-hoc binary are not assumed to transfer. Supported replacement of only the QA Accessibility entry was initiated in System Settings: the selected **Intent QA** row was verified through accessibility state before Remove, and macOS displayed a Touch ID/password authentication sheet. User authentication is pending. No completed removal or replacement is claimed; other applications' permission entries were not changed.

There is no active QA intention. A fresh read-only checkpoint confirmed unchanged hashes for all three daily intention/schedule/cooldown files, inactive QA browser rules and one saved QA intention. The exact new artifact remained the sole verified QA process, PID `77673`. These are checkpoint results during the authentication handoff, not final cleanup.
...
| 1 | Grant QA permissions and observe the guide's real refresh | Recovered for gZRuKyGJ before its native/Chrome runs. The later ksf8n4nv binary currently shows the real permission gate; supported QA-only entry replacement is paused at a macOS Touch ID/password sheet. No grant transfer or completed replacement is claimed. |
...
| Final | Cleanup, QA rules/processes/hosts, daily state hashes and daily app restoration | At the current authentication handoff, fresh checks confirm unchanged daily hashes, inactive QA rules and one saved QA intention; sole QA PID 77673 matches ksf8n4nv. Chrome Profile 7 extension/host, seven fixture tabs and server 18765 remain present. Final cleanup remains pending. |
