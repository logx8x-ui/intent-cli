...
## Follow-up browser and runtime checks

Continued on the same sole `buBsUPCq` QA process, PID 21890. The local fixture server had stopped; it was restarted on loopback port 18765 using the existing private QA pages. No personal browser profile or daily app was changed.

- **Live pass:** File → Safety Stop displayed “all active restrictions have been released” before enforcement testing.
- **Live pass:** The overview's explicit Chrome window chooser exposed all seven isolated-profile tabs. Selected Notes tab ID 1733679362 in Block mode and configured Duration = 1 minute through the real editor. Run displayed Quick Block at 01:00. Persisted rules contained only that tab ID, the matching browser session ID, and blacklist mode.
- **Live pass:** Chrome switched to the unblocked Tasks tab. Two UI attempts to activate the blocked Notes tab left Tasks active. All seven tab rows remained present. This proves the exercised browser activation guard, not physical mouse-event interception or full-screen composited blur.
- **Live pass:** Hiding running controls removed their content; the dashboard subsequently showed 00:03 with manual End disabled. After expiry it returned to idle. Notes could then be activated and its counter incremented from 2 to 3 without reloading its state. The expiry notice was not captured in this run, so no new notice-visibility pass is claimed.
- **Live partial:** The saved native one-minute Timer + Checklist fixture ran and showed readable 00:54/0-of-1 controls. Calculator remained available to inspect through AX. A later checkbox action returned noWindowsAvailable and rules were inactive afterward; this alone does not distinguish checklist completion from a near-deadline expiry, so it adds no checklist-first acceptance claim.
- **Automated pass:** `npm run test:extensions`, `node scripts/test-native-host.cjs`, and `node scripts/test-native-host-snapshot-refresh.cjs` all exited successfully. Log: `/tmp/intent-browser-followup-tests.log`. Includes Firefox/Chrome rules and background behaviour, Firefox installation checks, idle/reconnect limits and native snapshot freshness for both browsers.
- **Integrity pass:** Daily intentions, schedules and cooldown SHA-256 hashes still match `/tmp/intent-qa-daily-state-hashes.json`. QA rules are inactive. Gesture readiness remains trusted/ready with no Carbon fallback.

No implementation change or rebuild was needed for these checks. Physical double-mark/unmark on another QA tab, composited native blur/Spaces/input blocking, isolated Firefox live testing and the wider remaining matrix are still open. Browser enumeration exposes only personal Chrome and the in-app browser to browser automation; native Chrome can target the QA window, while earlier separate Firefox process binding remains unresolved. Do not substitute personal-profile testing or count extension unit tests as Firefox live acceptance. Fixtures remain for the physical handoff; no publication or cleanup completion is claimed.
