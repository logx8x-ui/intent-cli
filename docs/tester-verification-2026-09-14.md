...
- **Push-triggered publication verified on 15 September:** workflow `.github/workflows/beta-updates.yml` was activated through the owning GitHub session. Run https://github.com/logx8x-ui/intent-cli/actions/runs/34863818758 passed from push `fc5cb01` and published build `20260914154318` (`0.9.3-beta.2`). The initial run was killed while launching a Swift test executable; explicitly ad-hoc signing CI test executables resolved it, and all three test products passed. Sparkle then downloaded, installed and relaunched the CI-built app on this Mac; its deep signature passed and all three intention files retained their hashes. Cmd+G showed Chrome and Firefox tabs afterward and Escape closed it without starting a session.
...
- Passwordless email/code UI is implemented. The original Supabase project was resumed and its public Auth settings confirm email signup is enabled and verification is required. **Actual email delivery, code/link verification and account isolation/sync remain unverified.** The owning dashboard confirmed custom SMTP is OFF, so Supabase only delivers to project-team addresses and prevents template editing until a sender is configured. A real email-sending account and verified sender are required for friends. Both site URL and allowed redirect already include `intent://auth-callback`. The ownership SELECT policy checks `auth.uid() = user_id`; metadata queries confirmed authenticated SELECT/INSERT/UPDATE/DELETE privileges and no anonymous table grants. This is configuration inspection, not a two-account isolation test.
