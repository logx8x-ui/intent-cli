...
Recording Mode is optional and starts only after the user presses Start. It stores local aggregate totals for active application bundle identifiers and normalized browser domains for 24 hours, one week, or until the user stops it. It does not capture screenshots, screen video, keystrokes, document contents, window titles, URL paths, query strings, or search terms, so it does not request macOS Screen Recording permission. Browser domains are available only when the user has enabled Intent Browser Guard in that browser. Recording data stays in `~/.intent/activity-recording.json`, is restricted to the signed-in macOS user, is never account-synced or sent to Intent's hosted AI service, and can be deleted from Recording Mode. Suggestions are generated locally, limited to seven, always use Allow Only mode, and remain editable before they are added.
