...
async function load() {
  try { render(await browser.runtime.sendMessage({ type: "getGuardStatus" })); }
  catch (_) { statusText.textContent = "Connecting to Intent…"; }
}
checkbox.addEventListener("change", async () => {
  checkbox.disabled = true;
  try { render(await browser.runtime.sendMessage({ type: "setGuardEnabled", enabled: checkbox.checked })); }
