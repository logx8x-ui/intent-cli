#!/usr/bin/env python3
"""Prepare a copied Firefox guard and unique native host for an isolated QA profile.

No Firefox launch, extension installation, or production-manifest modification.
...
def prepare(root, source, host_binary, manifest_directory=None):
    ...
    guard_id = "intent-firefox-qa@loganmondi.dev"
    gecko = manifest.get("browser_specific_settings", {}).get("gecko")
    if not isinstance(gecko, dict) or not gecko.get("id"):
        raise ValueError("Expected Firefox extension identity in browser_specific_settings.gecko")
    ...
    try:
        ...
        receipt = {"root": str(root), "manifest": str(destination), "wrapper": str(wrapper),
                   "extension": str(guard), "profile": str(root / "firefox-profile"), "extension_id": guard_id}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cleanup", action="store_true", help="Remove only this run's QA registration and copied guard/host after stopping QA Firefox")
    args = parser.parse_args()
    if args.cleanup:
        cleanup(args.root)
        print("Removed this run's QA native-host registration and copied browser fixture; profile/data retained.")
    else:
        result = prepare(args.root, Path(__file__).resolve().parent.parent / "firefox-extension", args.host_binary)
